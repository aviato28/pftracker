...
this repo itself and can download + launch the installer for a newer
build — see `data/update/app_update_service.dart` and
...
on-demand from Settings → About → "Check for updates". The repo is
public, so this needs no authentication and no build-time secret.
...
2. Tag the commit `vX.Y.Z+N` — **matching pubspec.yaml exactly** — and
...
3. Any `.apk` among the release's assets is picked up — attaching just
